...
class _OrderHistoryState extends State<OrderHistory> {
  ...
  Future<void> cancelOrder(String orderId) async {
    try {
      CollectionReference postCollection = FirebaseFirestore.instance.collection('users_order_history').doc(user!.email).collection('Items');

      // Fetch data from the furniture_list collection
      QuerySnapshot postData = await postCollection.where('orderId', isEqualTo: orderId).get();
        ...
        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              final orderData = orderHistory[index].data() as Map<String, dynamic>;
              final List<Map<String, dynamic>> orderItems = List<Map<String, dynamic>>.from(orderData['Products']);
              ...
              return ListTile(
                ...
                subtitle: Column(
                  ...
                  children: [
                    ...
                    Text('Products:'),
                    for (var item in orderItems) Text('${item["name"]} - ${item["price"]}'),
                    ElevatedButton(
                      onPressed: () {
                      ...
                      child: Text('Cancel Order', style: TextStyle(color: Color(0xffffffff), fontFamily: 'RobotoSerif', fontSize: 16),),
                    ),
                  ],
                ),
              );
